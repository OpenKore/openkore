...
sub sendChat {
	...
	$data = pack("C*", 0xF3, 0x00) . pack("v*", length($charName) + length($message) + 8) . $charName . " : " . $message . chr(0);
...
sub sendDrop {
	...
	my $msg;
	$msg = pack("C*", 0x9B, 0x00) . pack("v1", $index) . pack("v1", $amount);
	$self->sendToServer($msg);
...
sub sendGetCharacterName {
	my ($self, $ID) = @_;
	my $msg;
	$msg = pack("C*", 0xF5, 0x00) . $ID . pack("x5");
	$self->sendToServer($msg);
...
sub sendGetPlayerInfo {
	my ($self, $ID) = @_;
	my $msg;
	$msg = pack("C*", 0x90, 0x01) . $ID . pack("x1");
	$self->sendToServer($msg);
...
sub sendItemUse {
	...
	my $targetID = shift;
	my $msg;
	$msg = pack("C*", 0x13, 0x01) . $targetID . pack("v",$ID);
	$self->sendToServer($msg);
	debug "Item Use: $ID\n", "sendPacket", 2;
}
...
sub sendLook {
	my ($self, $body, $head) = @_;
	my $msg;
	$msg = pack("C*", 0x72, 0) . pack("C*", $head, 0, $body);
	$self->sendToServer($msg);
...
sub sendMove {
	...
	my $msg;
	$msg = pack("C*", 0xF7, 0) . pack("x8") . pack("x3") . getCoordString($x, $y, 1);
	$self->sendToServer($msg);
...
sub sendSkillUseLoc {
	my ($self, $ID, $lv, $x, $y) = @_;
	my $msg;
	$msg = pack("C*", 0x9F, 0x00) . pack("v*", $y) . pack("x3") . pack("v", $lv) . pack("v*", $ID) . pack("v*", $x);
	$self->sendToServer($msg);
...
sub sendStorageAdd {
	...
	my $amount = shift;
	my $msg;
	$msg = pack("C*", 0xA7, 0x00) . pack("V", $amount) . pack("x1") . pack("v", $index);
	$self->sendToServer($msg);
...
sub sendStorageGet {
	...
	my $amount = shift;
	my $msg;
	$msg = pack("C*", 0x94, 0x00) . pack("v", $index) . pack("V*", $amount) . pack("x6");
	$self->sendToServer($msg);
