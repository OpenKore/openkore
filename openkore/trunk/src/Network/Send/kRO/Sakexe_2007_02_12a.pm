...
use strict;
use Network::Send::kRO::Sakexe_2007_02_05a;
...
sub sendMapLogin {
	...
	my $msg = pack('v x2 a4 x a4 x4 a4 V C', 0x009B, $accountID, $charID, $sessionID, getTickCount(), $sex);
	$self->sendToServer($msg);
}
...
sub sendDrop {
	my ($self, $index, $amount) = @_;
	my $msg = pack('v x3 v x v', 0x0116, $index, $amount);
	$self->sendToServer($msg);
