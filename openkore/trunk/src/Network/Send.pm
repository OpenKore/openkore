...
sub sendAttack {
	...
	} elsif (($config{serverType} == 14) || ($config{serverType} == 15)) { # pRO Thor (12/1/2006): packet 008C
		error "Your server is not supported because it uses padded packets.\n";
		if (AI::action() eq "NPC") {
			error "Failed to talk to monster NPC.\n";
			AI::dequeue();
...
sub sendChat {
	...
	if (($config{serverType} == 3) || ($config{serverType} == 5) || ($config{serverType} == 8) || ($config{serverType} == 9)) {
		...
		$data = pack("C*", 0x7E, 0x00) .
			pack("v*", length($charName) + length($message) + 8) .
			$charName . " : " . $message . chr(0);
 	} elsif ($config{serverType} == 14) {
		$data = pack("C*", 0x85, 0x00) .
			pack("v*", length($charName) + length($message) + 8) .
			$charName . " : " . $message . chr(0);
	} elsif ($config{serverType} == 15) {
		$data = pack("C*", 0xF3, 0x00) .
			pack("v*", length($charName) + length($message) + 8) .
			$charName . " : " . $message . chr(0);
	} else {
...
sub sendDrop {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0xA2, 0x00, 0x4B, 0x00, 0x98) .
		pack("v1", $index) .
		pack("C*", 0x03) .
		pack("v1", $amount);
	}
...
sub sendGetCharacterName {
	...
	if ($config{serverType} == 3) {
		...
		$msg = pack("C*", 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00) . $ID;
...
sub sendGetPlayerInfo {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0xF5, 0x00) . $ID . pack("C*", 0x00, 0x00, 0x00);
		...
		$msg = pack("C*", 0x94, 0x00) . pack("C*", 0x12, 0x05, 0x0C, 0x7B, 0x12) . $ID;
	}
...
sub sendItemUse {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0xA7, 0x00, 0x81, 0x06) . 
		pack("v*", $ID) .
...
sub sendLook {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0x90, 0x01, $body, $head, 0x00);
		...
		$msg = pack("C*", 0x9B, 0x00, 0x01, 0x02, 0x98, 0x35, 0x5D, $head, 0x00, 0xE8, $body);
	}
...
# 16>  64 63 6F 83 44 34 60 6B    0A 00
	...
	} elsif ($config{serverType} == 14) { #pRO Thor (12/1/2006): 34 bytes
		$msg = pack("C*", 0x9F, 0x00) .
			$accountID .
			pack("x3") .
			$charID .
	...
	} elsif ($config{serverType} == 15) { #pRO Thor (12/6/2006)
...
sub sendMove {
	...
	if (($config{serverType} == 3)) {
		...
		$msg = pack("C*", 0x85, 0x00, 0x5D, 0x03) . getCoordString($x, $y);
...
sub sendSit {
	...
	} elsif (($config{serverType} == 14) || ($config{serverType} == 15)) {
		error "Your server is not supported because it uses padded packets.\n";
		if (AI::action() eq "sitting") {
			error "Failed to sit.\n";
			AI::dequeue();
...
sub sendSkillUse {
	...
	} elsif (($config{serverType} == 14) || ($config{serverType} == 15)) { #pRO Thor
		error "Your server is not supported because it uses padded packets.\n";
		if (AI::action() eq 'teleport') {
			error "Failed to use teleport skill.\n";
			AI::dequeue();
...
#0010  68 05, F6 03 10 D4, 3B 00, D8 1A, B4 76, 5C 00          h.....;....v\.
		...
		$msg = pack("C*", 0x16, 0x01, 0x2C, 0x00, 0x1F) .
			pack("v", $lv) .
			pack("C*", 0x00, 0x98) .
			pack("v*", $ID) .
			pack("C*", 0x03) .
...
sub sendStorageAdd {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0xF3, 0x00, 0x6E, 0x05, 0x78, 0xD1, 0x00) .
			pack("v", $index) .
			pack("C*", 0xE5) .
			pack("V", $amount);
	}
...
sub sendStorageClose {
	my $msg;
	if (($config{serverType} == 3) || ($config{serverType} == 5) || ($config{serverType} == 8) || ($config{serverType} == 9) || ($config{serverType} == 15)) {
...
sub sendStorageGet {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0xF5, 0x00, 0x00, 0x00, 0x10, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00) .
			pack("v*", $index) .
			pack("C*", 0x21, 0x7E) .
			pack("V*", $amount);
	}
...
sub sendSync {
	...
	} elsif ($config{serverType} == 14) { #pRO Thor (12/1/2006)
...
sub sendTake {
	...
	if ($config{serverType} == 0) {
		...
		$msg = pack("C*", 0x9F, 0x00, 0x5D, 0x03) . $itemID;
	}
