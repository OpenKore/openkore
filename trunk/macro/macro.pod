...
=item C<checkCart(condition)>

Checks whether cart matches I<condition>. I<condition> is
I<"item name"> I<condition> I<amount>.
...
=item C<checkInventory(condition)>

Checks whether inventory matches I<condition>. I<condition> is
I<"item name"> I<condition> I<amount>.

Example: C<checkInventory("\"red potion\" >= 100")>
...
=item C<checkSpirits(condition)>

Checks whether spirits match I<condition>.

Example: C<checkSpirits("> 3")>
...
=item C<checkNPCTalk(message)>

Checks wheter an NPC sent the message specified by I<message> which is
a regexp.

Example: C<checkNPCTalk("^Trigger Message$")>
...
=item C<checkPubM(publine, r_hash)>

Checks whether a public chat message was received. I<publine> has the
following syntax: "/trigger message or regexp/,distance", I<r_hash> is
a reference to a hash containing I<pubMsgID>, I<pubMsgUser> and I<pubMsg>.
This function sets the variable I<lastPubNick> which can be accessed
either via C<getVar("I<variable>")> or directly with $varStack{variable}.

Example: C<checkPubM("/^my trigger message$/,5", $arg)>

Returns: 1 (true), 0 (false)

=item C<checkShop(condition)>

Checks whether shop matches I<condition>. I<condition> is
I<"item name"> I<condition> I<amount>.

Example: C<checkShop("\"red potion\" >= 100")>
...
Returns: 1 if AI is either "macro" or "deal" or $automacro{overrideAI}
is set, else 0
