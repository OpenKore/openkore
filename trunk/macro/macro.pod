...
Array of macro IDs that are to be ignored. Usually run-once automacro IDs get in here.
...
'/regexp/,nick1,nick2,nick3,..', or '"trigger message",nick1,nick2,nick3,...'
The nick arguments are optional.
I<r_hash> is a reference to a hash containing I<privMsgUser> and I<privMsg>.
This function sets the variable I<lastPMnick> which can be accessed either
via C<getVar("I<variable>")> or directly with $varStack{variable}.

Example: C<checkPM("/^my trigger message$/,Tom Jones", $arg)> (same as:
C<checkPM("\"my trigger message\",Tom Jones", $arg)>)
...
following syntax: '/regexp/,distance' or '"trigger message",distance',
The distance argument is optional.
I<r_hash> is a reference to a hash containing I<pubMsgID>, I<pubMsgUser>
and I<pubMsg>.
...
Example: C<checkPubM("/^my trigger message$/,5", $arg)> (same as:
C<checkPubM("\"my trigger message\",5", $arg)>
...
Then clears all hashes and hooks and removes macro.txt from C<Settings>.
