...
sub validate_condition {
	...
	if ($callback_type eq 'variable') {
		...
		if ($self->{hook_type} eq 'packet_chatinfo' && !defined $self->{fulfilled_chatID} && $self->validator_check($args->{title})) {
			$self->{fulfilled_chatID} = $args->{chatID};
			
		} elsif ($self->{hook_type} eq 'chat_created' && !defined $self->{fulfilled_chatID} && $self->validator_check($args->{chat}{title})) {
			$self->{fulfilled_chatID} = $accountID;

		} elsif ($self->{hook_type} eq 'chat_removed' && defined $self->{fulfilled_chatID} && $args->{ID} eq $self->{fulfilled_chatID}) {
			my $last_id = $$self->{fulfilled_chatID};
			$self->recheck_all_chat_names($last_id);
		
		} elsif ($self->{hook_type} eq 'chat_modified') {
