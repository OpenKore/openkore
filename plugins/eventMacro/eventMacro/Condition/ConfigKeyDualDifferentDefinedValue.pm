package eventMacro::Condition::ConfigKeyDualDifferentDefinedValue;
...
sub _parse_syntax {
	...
	$self->{fulfilled_value1} = undef;
	$self->{fulfilled_value2} = undef;
...
sub validate_condition {
	...
	return $self->SUPER::validate_condition( (defined $self->{fulfilled_value1} ? 1 : 0) );
}
...
sub check_keys {
	...
	return if ($config{$self->{key_1}} eq $config{$self->{key_2}});
	
	$self->{fulfilled_value1} = $config{$self->{key_1}};
	$self->{fulfilled_value2} = $config{$self->{key_2}}
}
...
sub get_new_variable_list {
	...
	$new_variables->{".".$self->{name}."LastValue1"} = $self->{fulfilled_value1};
	$new_variables->{".".$self->{name}."LastValue2"} = $self->{fulfilled_value2};
