...
#my $packetHook1 = Plugins::addHook('packet_pre/actor_display', \&manual);
#my $packetHook2 = Plugins::addHook('parseMsg/addPrivMsgUser', \&private);

my $packetHook = Plugins::addHooks (
	['self_died', \&death, undef],
	['packet_pre/actor_display', \&monster, undef],
	['parseMsg/addPrivMsgUser', \&private, undef],
);
#sub Reload {
...
sub death {
#eventList death
message "eventList death ($config{'alertSound'})\n";
...
#			message " Actor is a pet - $display\n"
#	} else {
#	message "Actor is a monster - $display\n"
#	}
	}
}
sub private {
...
#	return if (!$config{'alertSound'});
	my $hookName = shift;
	my $args = shift;
	my $privMsgUser = $args->{user};
message "PM from: $privMsgUser ($config{'alertSound'})/n";
return if (!$config{'alertSound'});
...
#message " 0078 # Existance packet used to tell if monster exists\n";
#				my $ID = substr($msg, 2, 4);
#                my $type = unpack("S*",substr($msg, 15, 4));
#                my $pet = unpack("C*",substr($msg, 16,  1));
#message "ID = $ID, type = $type, pet = $pet\n";
...
#message " 01D8 # Existance packet used to tell if monster exists\n";
#                my $ID = substr($msg, 2, 4);
