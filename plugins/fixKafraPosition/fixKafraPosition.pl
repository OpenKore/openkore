...
#########
# startup
Plugins::register('fixKafraPosition', '', \&Unload, \&Unload);
...
sub onmissStorage {
	...
	if (defined $closest) {
		warning TF("[fixKafraPosition] [storage] Guessing our desired kafra to be %s (%s,%s).\n", $closest, $closest_x, $closest_y), "system";
		$args->{x} = $closest_x;
		$args->{y} = $closest_y;
		$args->{return} = 1;
	}
...
sub onmissTele {
	...
	if (defined $closest_portal_binID) {
		warning TF("[fixKafraPosition] [teleport] Guessing our desired kafra to be %s (%s,%s).\n", $closest, $closest_x, $closest_y), "system";
		$args->{x} = $closest_x;
		$args->{y} = $closest_y;
		$args->{return} = 1;
	}
