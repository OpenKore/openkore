...
public:
	WinServerSocket(const wxChar *address, unsigned short port) {
		...
		if (bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == SOCKET_ERROR) {
			wxString message;
			int error = WSAGetLastError();
			message.Printf(wxT("Cannot bind to %s (error %d)"),
				       address, error);
			closesocket(fd);
			throw SocketException(message, error);
		}
		...
		if (listen(fd, DEFAULT_BACKLOG_SIZE) == SOCKET_ERROR) {
			wxString message;
			int error = WSAGetLastError();
			message.Printf(wxT("Cannot listen for connections on socket (error %d)"),
				       error);
			closesocket(fd);
			throw SocketException(message, error);
		}
