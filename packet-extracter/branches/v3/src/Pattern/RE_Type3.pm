...
sub new {
	...
	$self->{map_pointer_pattern} = "83 ?? ?? 8D ?? ?? 89 ?? ?? C7 45 ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B ?? C7 ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B ?? ?? 8B C6"; # Pattern to find Function Pointer
	$self->{map_pointer_entry_offset} = 31; # Offset in Pattern to get the
...
sub onProcess {
	...
	$map_found = 1;
	print "Packet Len map found. Type: RE #3\n";
