...
sub new {
	...
	$self->{Hooks} = ['packet/stats_info', 'packet/stat_info'];
...
sub validate_condition_status {
	...
	$self->{is_Fulfilled} = validate_code_number_operator_compare_number_or_variable($char->{points_free}, $self->{Code_Operator}, $self->{Code_Number}, (@{$self->{Variables}} > 0 ? 1 : 0));
}
