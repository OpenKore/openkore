...
sub validate_condition_status {
	...
	if ($event_name eq 'variable_event') {
		$self->{is_Fulfilled} = cmpr($char->{lv}, $self->{Code_Condition}, $args->{'variable_value'});
	} else {
		if (@{$self->{Variables}} > 0) {
			my $variable_value = $eventMacro->get_var($self->{Code_Level});
			if (defined $variable_value) {
				$self->{is_Fulfilled} = cmpr($char->{lv}, $self->{Code_Condition}, $variable_value);
			} else {
				$self->{is_Fulfilled} = 0;
			}
		} else {
			$self->{is_Fulfilled} = cmpr($char->{lv}, $self->{Code_Condition}, $self->{Code_Level});
		}
	}
}
