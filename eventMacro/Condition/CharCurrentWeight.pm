...
sub validate_condition_status {
	...
	return if ($event_name eq 'packet/stat_info' && $args && $args->{type} != 24);
	
	$self->{is_Fulfilled} = validate_code_number_operator_compare_number_or_variable($char->{weight}, $self->{Code_Operator}, $self->{Code_Number}, (@{$self->{Variables}} > 0 ? 1 : 0));
}
