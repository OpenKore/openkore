...
sub extract {
    ...
    while ( !$fp->eof && @$files ) {
        my $file = shift @$files;
        for ( my $bytes = $file->{start} - tell( $fp ) ; $bytes > 0 ; ) { $bytes -= read $fp, $buf = '', $bytes > 8192 ? 8192 : $bytes; }
