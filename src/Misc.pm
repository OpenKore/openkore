...
sub useTeleport {
	...
	if ($use_lvl == 1) { #Fly Wing
		if (!$config{teleportAuto_item1}) {
			...
			$item = $char->inventory->getByNameID($config{teleportAuto_item1}) if (!($item) && length($config{teleportAuto_item1}) >= 3);
		}
		...
		if (!$config{teleportAuto_item2}) {
			...
			$item = $char->inventory->getByNameID($config{teleportAuto_item2}) if (!($item) && length($config{teleportAuto_item2}) >= 3);
		}
	}
