...
sub route {
	...
	my @params = (
		...
		map { $_ => $args{$_} } qw(targetNpcPos distFromGoal pyDistFromGoal notifyUponArrival avoidWalls randomFactor useManhattan singleMovePacket solution immediateStart)
	);
	...
	$task->{$_} = $args{$_} for qw(targetNpcPos attackID sendAttackWithMove attackOnRoute noSitAuto LOSSubRoute meetingSubRoute isRandomWalk isFollow isIdleWalk isSlaveRescue isMoveNearSlave isEscape isItemTake isItemGather isDeath isToLockMap runFromTarget singleMovePacket solution immediateStart);

	$self->queue('route', $task);
	$self->processTask('route') if $args{immediateStart};
}
