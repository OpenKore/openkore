...
sub unknownMessage {
	...
	if($masterServer->{serverType} eq 'tRO')
	{
		...
		if(( $h2int >= 2129 && $h2int <= 2448) && ($net->getState() == Network::IN_GAME)) #ถ้า Unknow Packet มีค่าตั้งแต่ 0851 - 0990 หรือแปลงเป็นตัวเลขได้ตั้งแต่ 2138 - 2448 ให้ทำงานในบล็อคด้านล่าง
		{
