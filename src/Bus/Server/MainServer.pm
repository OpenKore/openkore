...
# - multiTO - A comma-separated list of client IDs who are to receive this
#             message. If specified, it indicates that this message should be
#             privately fanned out to multiple recipients by the server.
#        If neither TO nor multiTO is specified, then this is a global
#        message, and will be broadcasted to all clients.
...
sub messageReceived {
	...
	if (my $handler = $self->can("process$MID")) {
		...
		if ($recepient) {
			...
			$self->log("Delivering message from $client->{name} to $recepientName\n");
			my %delivery_args = %{$args};
			delete $delivery_args{TO};
			$delivery_args{FROM} = $client->{ID};
			if ($self->send($args->{TO}, $MID, \%delivery_args) != 1) {
	...
	} elsif (exists $args->{multiTO}) {
		my @target_ids = grep { defined $_ && $_ ne '' } split(',', ($args->{multiTO} // ''));
		my %seen;
		foreach my $target_id (@target_ids) {
			next if $seen{$target_id}++;

			my $recepient = $self->getBusClient($target_id);
			if ($recepient) {
				my $recepientName = $recepient->{name};
				$self->log("Delivering multiTO message from $client->{name} to $recepientName\n");
				my %delivery_args = %{$args};
				delete $delivery_args{multiTO};
				$delivery_args{FROM} = $client->{ID};
				if ($self->send($target_id, $MID, \%delivery_args) != 1) {
					my %args2 = ( clientID => $target_id );
					$args2{SEQ} = $args->{SEQ} if (exists $args->{SEQ});
					$args2{IRY} = 1;
					$self->send($client->{ID}, 'DELIVERY_FAILED', \%args2);
				}
