	...
	if ($map_or_portal eq "stop") {
		...
		if ($currentChatRoom ne "") {
			...
			if ($map_or_portal =~ /^\d+$/) {
				...
				$file = File::Spec->catfile(Settings::getFieldsFolders(), $file) if (Settings::getFieldsFolders());
...
sub cmdPortalList {
	...
	if ($arg eq '') {
		...
		$srcfile = File::Spec->catfile(Settings::getFieldsFolders(), $srcfile) if (Settings::getFieldsFolders());
		$srcfile .= ".gz" if (! -f $srcfile); # compressed file
		my $dstfile = $dstMap.'.fld2';
		$dstfile = File::Spec->catfile(Settings::getFieldsFolders(), $dstfile) if (Settings::getFieldsFolders());
