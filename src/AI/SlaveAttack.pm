...
sub process {
	...
	if ($slave->action eq "attack") {
		my $ID = $slave->args->{ID};
		if (targetGone($slave)) {
			finishAttacking($slave);
		} elsif (shouldGiveUp($slave)) {
			giveUp($slave);
		} else {
			if (timeOut($slave->args->{attackMainTimeout}, 0.1)) {
...
sub shouldGiveUp {
	my $slave = shift;
	my $args = $slave->args;
	return !$config{$slave->{configPrefix}.'attackNoGiveup'} && (timeOut($slave->args->{ai_attack_giveup}) || $slave->args->{unstuck}{count} > 5)
}
...
sub targetGone {
	my $slave = shift;
	return !$monsters{$slave->args->{ID}} && (!$players{$slave->args->{ID}} || $players{$slave->args->{ID}}{dead})
}
	...
	if ($slave->action eq "attack") {
		...
		if (!$cleanMonster) {
			...
			if ($cell) {
				...
				$slave->route(undef, @{$cell}{qw(x y)}, noMapRoute => 1, avoidWalls => 0);
			} else {
